# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `countries` gem.
# Please instead update this file by running `bin/tapioca gem countries`.


# pkg:gem/countries#lib/countries/version.rb:3
module Countries; end

# pkg:gem/countries#lib/countries/version.rb:4
Countries::VERSION = T.let(T.unsafe(nil), String)

# pkg:gem/countries#lib/countries/configuration.rb:3
module ISO3166
  class << self
    # pkg:gem/countries#lib/countries/country/class_methods.rb:8
    def Country(country_data_or_country); end

    # pkg:gem/countries#lib/countries/configuration.rb:8
    def configuration; end

    # pkg:gem/countries#lib/countries/configuration.rb:5
    def configuration=(_arg0); end

    # pkg:gem/countries#lib/countries/configuration.rb:17
    def configure; end

    # pkg:gem/countries#lib/countries/configuration.rb:12
    def reset; end
  end
end

# pkg:gem/countries#lib/countries/configuration.rb:21
class ISO3166::Configuration
  # pkg:gem/countries#lib/countries/configuration.rb:24
  def initialize; end

  # Enables the integration with the {Money}[https://github.com/RubyMoney/money] gem
  #
  # Please note that it requires you to add "money" gem to your gemfile.
  #
  #   gem "money", "~> 6.9"
  #
  # *WARNING* if you have a top level class named +Money+ you will conflict with this gem.
  #
  # @example
  #   c = ISO3166::Country['us']
  #   c.currency.iso_code # => 'USD'
  #   c.currency.name # => 'United States Dollar'
  #   c.currency.symbol # => '$'
  #
  # pkg:gem/countries#lib/countries/configuration.rb:53
  def enable_currency_extension!; end

  # pkg:gem/countries#lib/countries/configuration.rb:22
  def loaded_locales; end

  # pkg:gem/countries#lib/countries/configuration.rb:22
  def loaded_locales=(_arg0); end

  # pkg:gem/countries#lib/countries/configuration.rb:28
  def locales; end

  # pkg:gem/countries#lib/countries/configuration.rb:32
  def locales=(value); end

  private

  # :reek:UtilityFunction
  #
  # pkg:gem/countries#lib/countries/configuration.rb:61
  def default_locales; end
end

# pkg:gem/countries#lib/countries/country/conversion_methods.rb:4
module ISO3166::ConversionMethods
  # @param alpha2 [String] ISO 3166 alpha-2 country code.
  # @return [String] ISO 3166 alpha-3 country code.
  #
  # pkg:gem/countries#lib/countries/country/conversion_methods.rb:7
  def from_alpha2_to_alpha3(alpha2); end

  # @param alpha3 [String] ISO 3166 alpha-3 country code.
  # @return [String] ISO 3166 alpha-2 country code.
  #
  # pkg:gem/countries#lib/countries/country/conversion_methods.rb:13
  def from_alpha3_to_alpha2(alpha3); end
end

# :reek:TooManyMethods
#
# pkg:gem/countries#lib/countries/country.rb:5
class ISO3166::Country
  include ::ISO3166::Emoji
  include ::ISO3166::CountrySubdivisionMethods
  include ::ISO3166::TimezoneExtensions
  extend ::ISO3166::CountryClassMethods
  extend ::ISO3166::ConversionMethods
  extend ::ISO3166::CountryFinderMethods

  # pkg:gem/countries#lib/countries/country.rb:25
  def initialize(country_data); end

  # pkg:gem/countries#lib/countries/country.rb:55
  def <=>(other); end

  # :reek:FeatureEnvy
  # :reek:ManualDispatch
  #
  # pkg:gem/countries#lib/countries/country.rb:43
  def ==(other); end

  # Access country data by key, symbol or string.
  # :reek:ManualDispatch
  #
  # pkg:gem/countries#lib/countries/country.rb:61
  def [](key); end

  # pkg:gem/countries#lib/countries/country.rb:14
  def address_format; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def alpha2; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def alpha3; end

  # pkg:gem/countries#lib/countries/country.rb:20
  def bounds; end

  # @return [String] the “common name” of this Country in English.
  #
  # pkg:gem/countries#lib/countries/country.rb:143
  def common_name; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def continent; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def country_code; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def currency_code; end

  # pkg:gem/countries#lib/countries/country.rb:11
  def data; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def distance_unit; end

  # pkg:gem/countries#lib/countries/country.rb:47
  def eql?(other); end

  # +true+ if this country is a member of the European Economic Area or it is UK
  #
  # pkg:gem/countries#lib/countries/country.rb:85
  def gdpr_compliant?; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def gec; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def geo; end

  # pkg:gem/countries#lib/countries/country.rb:51
  def hash; end

  # +true+ if this country is a member of the European Economic Area.
  # :reek:NilCheck :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country.rb:91
  def in_eea?; end

  # +true+ if this country is a member of the European Single Market.
  # :reek:NilCheck :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country.rb:97
  def in_esm?; end

  # +true+ if this country is a member of the European Union.
  # :reek:NilCheck :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country.rb:68
  def in_eu?; end

  # +true+ if this country is a member of the EU VAT Area.
  # :reek:NilCheck :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country.rb:103
  def in_eu_vat?; end

  # +true+ if this country is a member of the G20.
  # :reek:NilCheck :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country.rb:80
  def in_g20?; end

  # +true+ if this country is a member of the G7.
  # :reek:NilCheck :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country.rb:74
  def in_g7?; end

  # +true+ if this country is a member of the United Nations.
  # :reek:NilCheck :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country.rb:109
  def in_un?; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def international_prefix; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def ioc; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def iso_long_name; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def iso_short_name; end

  # @return [String] The ISO 3166-1 "Short name lower case" value for this Country.
  #
  # pkg:gem/countries#lib/countries/country.rb:114
  def iso_short_name_lower_case; end

  # pkg:gem/countries#lib/countries/country.rb:39
  def languages; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def languages_official; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def languages_spoken; end

  # pkg:gem/countries#lib/countries/country.rb:20
  def latitude; end

  # @return [String] The name for this Country, in this Country's locale.
  #
  # pkg:gem/countries#lib/countries/country.rb:157
  def local_name; end

  # @return [Array<String>] The list of names for this Country, in this Country's locales.
  #
  # pkg:gem/countries#lib/countries/country.rb:149
  def local_names; end

  # pkg:gem/countries#lib/countries/country.rb:20
  def longitude; end

  # pkg:gem/countries#lib/countries/country.rb:20
  def max_latitude; end

  # pkg:gem/countries#lib/countries/country.rb:20
  def max_longitude; end

  # pkg:gem/countries#lib/countries/country.rb:20
  def min_latitude; end

  # pkg:gem/countries#lib/countries/country.rb:20
  def min_longitude; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def nanp_prefix; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def national_destination_code_lengths; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def national_number_lengths; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def national_prefix; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def nationality; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def number; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def postal_code; end

  # pkg:gem/countries#lib/countries/country.rb:37
  def postal_code?; end

  # @return [String] The regex for valid postal codes in this Country
  #
  # pkg:gem/countries#lib/countries/country.rb:14
  def postal_code_format; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def region; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def start_of_week; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def subregion; end

  # pkg:gem/countries#lib/countries/country.rb:123
  def to_s; end

  # @return [Array<String>] the list of names for this Country in all loaded locales.
  #
  # pkg:gem/countries#lib/countries/country.rb:128
  def translated_names; end

  # @param locale [String] The locale to use for translations.
  # @return [String] the name of this Country in the selected locale.
  # :reek:FeatureEnvy
  #
  # pkg:gem/countries#lib/countries/country.rb:135
  def translation(locale = T.unsafe(nil)); end

  # pkg:gem/countries#lib/countries/country.rb:14
  def translations; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def un_locode; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def unofficial_names; end

  # :reek:NilCheck
  #
  # pkg:gem/countries#lib/countries/country.rb:31
  def valid?; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def vat_rates; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def vehicle_registration_code; end

  # pkg:gem/countries#lib/countries/country.rb:14
  def world_region; end

  # pkg:gem/countries#lib/countries/country.rb:35
  def zip; end

  # pkg:gem/countries#lib/countries/country.rb:36
  def zip?; end

  # pkg:gem/countries#lib/countries/country.rb:38
  def zip_format; end

  private

  # pkg:gem/countries#lib/countries/country.rb:250
  def reload; end
end

# pkg:gem/countries#lib/countries/country/class_methods.rb:19
module ISO3166::CountryClassMethods
  # pkg:gem/countries#lib/countries/country/class_methods.rb:34
  def all(&block); end

  # :reek:UtilityFunction
  # :reek:ManualDispatch
  #
  # pkg:gem/countries#lib/countries/country/class_methods.rb:43
  def all_names_with_codes(locale = T.unsafe(nil)); end

  # pkg:gem/countries#lib/countries/country/class_methods.rb:54
  def all_translated(locale = T.unsafe(nil)); end

  # :reek:UtilityFunction
  #
  # pkg:gem/countries#lib/countries/country/class_methods.rb:30
  def codes; end

  # @param query_val [String] A value to query using `query_method`
  # @param query_method [Symbol] An optional query method, defaults to Country#alpha2
  # @param result_method [Symbol] An optional method of `Country` to apply to the result set.
  # @return [Array] An array of countries matching the provided query, or the result of applying `result_method` to the array of `Country` objects
  #
  # pkg:gem/countries#lib/countries/country/class_methods.rb:74
  def collect_countries_with(query_val, query_method = T.unsafe(nil), result_method = T.unsafe(nil)); end

  # @param subdivision_str [String] A subdivision name or code to search for. Search includes translated subdivision names.
  # @param result_method [Symbol] An optional method of `Country` to apply to the result set.
  # @return [Array] An array of countries with subdivisions matching the provided name, or the result of applying `result_method` to the array of `Country` objects
  #
  # pkg:gem/countries#lib/countries/country/class_methods.rb:84
  def collect_likely_countries_by_subdivision_name(subdivision_str, result_method = T.unsafe(nil)); end

  # pkg:gem/countries#lib/countries/country/class_methods.rb:39
  def countries(&block); end

  # pkg:gem/countries#lib/countries/country/class_methods.rb:20
  def new(country_data); end

  # pkg:gem/countries#lib/countries/country/class_methods.rb:50
  def pluck(*attributes); end

  # :reek:UtilityFunction
  # :reek:FeatureEnvy
  #
  # pkg:gem/countries#lib/countries/country/class_methods.rb:60
  def translations(locale = T.unsafe(nil)); end

  protected

  # Some methods like parse_value are expensive in that they
  # create a large number of objects internally. In order to reduce the
  # object creations and save the GC, we can cache them in an class instance
  # variable. This will make subsequent parses O(1) and will stop the
  # creation of new String object instances.
  #
  # NB: We only want to use this cache for values coming from the JSON
  # file or our own code, caching user-generated data could be dangerous
  # since the cache would continually grow.
  # :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country/class_methods.rb:112
  def cached(value); end

  # :reek:UtilityFunction
  #
  # pkg:gem/countries#lib/countries/country/class_methods.rb:94
  def strip_accents(string); end

  private

  # :reek:UtilityFunction
  #
  # pkg:gem/countries#lib/countries/country/class_methods.rb:122
  def custom_countries_translations(locale); end
end

# pkg:gem/countries#lib/countries/country/finder_methods.rb:4
module ISO3166::CountryFinderMethods
  # pkg:gem/countries#lib/countries/country/finder_methods.rb:17
  def [](query); end

  # :reek:NestedIterators
  #
  # pkg:gem/countries#lib/countries/country/finder_methods.rb:22
  def find_all_by(attribute, val); end

  # :reek:FeatureEnvy
  #
  # pkg:gem/countries#lib/countries/country/finder_methods.rb:36
  def method_missing(method_name, *arguments); end

  # :reek:FeatureEnvy
  #
  # pkg:gem/countries#lib/countries/country/finder_methods.rb:9
  def search(query); end

  protected

  # :reek:ControlParameter
  #
  # pkg:gem/countries#lib/countries/country/finder_methods.rb:59
  def find_by(attribute, value, obj = T.unsafe(nil)); end

  # pkg:gem/countries#lib/countries/country/finder_methods.rb:65
  def parse_attributes(attribute, val); end

  # :reek:ManualDispatch
  # :reek:FeatureEnvy
  #
  # pkg:gem/countries#lib/countries/country/finder_methods.rb:77
  def parse_value(value); end

  # pkg:gem/countries#lib/countries/country/finder_methods.rb:82
  def searchable_attribute?(attribute); end

  # pkg:gem/countries#lib/countries/country/finder_methods.rb:86
  def searchable_attributes; end

  private

  # :reek:BooleanParameter
  #
  # pkg:gem/countries#lib/countries/country/finder_methods.rb:47
  def respond_to_missing?(method_name, include_private = T.unsafe(nil)); end
end

# pkg:gem/countries#lib/countries/country/finder_methods.rb:5
ISO3166::CountryFinderMethods::FIND_BY_REGEX = T.let(T.unsafe(nil), Regexp)

# pkg:gem/countries#lib/countries/country/finder_methods.rb:6
ISO3166::CountryFinderMethods::SEARCH_TERM_FILTER_REGEX = T.let(T.unsafe(nil), Regexp)

# pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:4
module ISO3166::CountrySubdivisionMethods
  # @param subdivision_str [String] A subdivision name or code to search for. Search includes translated subdivision names.
  # @return [Subdivision] The first subdivision matching the provided string
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:7
  def find_subdivision_by_name(subdivision_str); end

  # @return [Array<String>] the list of humanized subdivision types for this country. Uses ActiveSupport's `#humanize` if available
  # :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:48
  def humanized_subdivision_types; end

  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:15
  def subdivision_for_string?(subdivision_str); end

  # @param locale [String] The locale to use for translations.
  # @return [Array<String>] A list of subdivision names for this country.
  # :reek:FeatureEnvy
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:66
  def subdivision_names(locale = T.unsafe(nil)); end

  # @param locale [String] The locale to use for translations.
  # @return [Array<Array>] This Country's subdivision pairs of names and codes.
  # :reek:FeatureEnvy
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:59
  def subdivision_names_with_codes(locale = T.unsafe(nil)); end

  # @return [Array<String>] the list of subdivision types for this country
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:42
  def subdivision_types; end

  # @return [Array<ISO3166::Subdivision>] the list of subdivisions for this Country.
  # :reek:DuplicateMethodCall
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:27
  def subdivisions; end

  #  +true+ if this Country has any Subdivisions.
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:21
  def subdivisions?; end

  # @param types [Array<String>] The locale to use for translations.
  # @return [Array<ISO3166::Subdivision>] the list of subdivisions of the given type(s) for this Country.
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:37
  def subdivisions_of_types(types); end

  private

  # :reek:UtilityFunction
  #
  # pkg:gem/countries#lib/countries/country/country_subdivision_methods.rb:73
  def humanize_string(str); end
end

# pkg:gem/countries#lib/countries/structure.rb:4
ISO3166::DEFAULT_COUNTRY_HASH = T.let(T.unsafe(nil), Hash)

# pkg:gem/countries#lib/countries/structure.rb:61
ISO3166::DEFAULT_SUBDIVISION_HASH = T.let(T.unsafe(nil), Hash)

# Handles building the in memory store of countries data
#
# pkg:gem/countries#lib/countries/data.rb:5
class ISO3166::Data
  extend ::ISO3166::SubdivisionMethods
  extend ::ISO3166::LocalesMethods

  # pkg:gem/countries#lib/countries/data.rb:16
  def initialize(alpha2); end

  # pkg:gem/countries#lib/countries/data.rb:23
  def call; end

  class << self
    # pkg:gem/countries#lib/countries/data.rb:47
    def cache; end

    # pkg:gem/countries#lib/countries/data.rb:61
    def codes; end

    # pkg:gem/countries#lib/countries/data.rb:77
    def datafile_path(file_array); end

    # pkg:gem/countries#lib/countries/data.rb:72
    def loaded_codes; end

    # Registers a new Country with custom data.
    # If you are overriding an existing country, this does not perform a deep merge so you will need to __bring in all data you wish to be available__.
    # Overriding an existing country will also remove it from the internal management of translations.
    #
    # pkg:gem/countries#lib/countries/data.rb:31
    def register(data); end

    # Resets the loaded data and cache
    #
    # pkg:gem/countries#lib/countries/data.rb:52
    def reset; end

    # Removes a country from the loaded data
    #
    # pkg:gem/countries#lib/countries/data.rb:41
    def unregister(alpha2); end

    # pkg:gem/countries#lib/countries/data.rb:66
    def update_cache; end

    private

    # pkg:gem/countries#lib/countries/data.rb:123
    def cache_flush_required?; end

    # pkg:gem/countries#lib/countries/data.rb:114
    def cached_codes; end

    # pkg:gem/countries#lib/countries/data.rb:162
    def deep_stringify_keys(data); end

    # Codes that we have translations for in dataset
    #
    # pkg:gem/countries#lib/countries/data.rb:119
    def internal_codes; end

    # pkg:gem/countries#lib/countries/data.rb:137
    def load_alpha2_translation_for_locale(alpha2, locale, locale_names); end

    # pkg:gem/countries#lib/countries/data.rb:157
    def load_cache(file_array); end

    # pkg:gem/countries#lib/countries/data.rb:83
    def load_data!; end

    # pkg:gem/countries#lib/countries/data.rb:110
    def load_required?; end

    # pkg:gem/countries#lib/countries/data.rb:127
    def load_translations(locale); end

    # pkg:gem/countries#lib/countries/data.rb:169
    def subdivision_file_path(alpha2); end

    # pkg:gem/countries#lib/countries/data.rb:94
    def sync_translations!; end

    # pkg:gem/countries#lib/countries/data.rb:106
    def synchronized(&_arg0); end

    # pkg:gem/countries#lib/countries/data.rb:152
    def unload_alpha2_translation_for_locale(alpha2, locale); end

    # pkg:gem/countries#lib/countries/data.rb:143
    def unload_translations(locale); end
  end
end

# pkg:gem/countries#lib/countries/country/emoji.rb:4
module ISO3166::Emoji
  # @return [String] the emoji flag for this country
  #
  # The emoji flag for this country, using Unicode Regional Indicator characters. e.g: "U+1F1FA U+1F1F8" for 🇺🇸
  #
  # pkg:gem/countries#lib/countries/country/emoji.rb:37
  def emoji_flag; end
end

# pkg:gem/countries#lib/countries/country/emoji.rb:5
ISO3166::Emoji::CODE_POINTS = T.let(T.unsafe(nil), Hash)

# pkg:gem/countries#lib/countries/data/locales_methods.rb:4
module ISO3166::LocalesMethods
  private

  # :reek:UtilityFunction
  #
  # pkg:gem/countries#lib/countries/data/locales_methods.rb:21
  def loaded_locales; end

  # pkg:gem/countries#lib/countries/data/locales_methods.rb:7
  def locales_to_load; end

  # pkg:gem/countries#lib/countries/data/locales_methods.rb:11
  def locales_to_remove; end

  # :reek:UtilityFunction
  #
  # pkg:gem/countries#lib/countries/data/locales_methods.rb:16
  def requested_locales; end
end

# pkg:gem/countries#lib/countries/subdivision.rb:4
class ISO3166::Subdivision
  # pkg:gem/countries#lib/countries/subdivision.rb:13
  def initialize(attrs); end

  # pkg:gem/countries#lib/countries/subdivision.rb:19
  def [](attr_name); end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def code; end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def code=(_arg0); end

  # @return [Hash] A hash with the subdivision code as key and the hash of translated subdivision names ( locale => name ) as value.
  #
  # pkg:gem/countries#lib/countries/subdivision.rb:24
  def code_with_translations; end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def comments; end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def comments=(_arg0); end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def geo; end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def geo=(_arg0); end

  # pkg:gem/countries#lib/countries/subdivision.rb:28
  def match?(subdivision_str); end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def name; end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def name=(_arg0); end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def translations; end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def translations=(_arg0); end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def type; end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def type=(_arg0); end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def unofficial_names; end

  # pkg:gem/countries#lib/countries/subdivision.rb:5
  def unofficial_names=(_arg0); end
end

# pkg:gem/countries#lib/countries/data/subdivision_methods.rb:4
module ISO3166::SubdivisionMethods
  # :reek:UtilityFunction
  #
  # pkg:gem/countries#lib/countries/data/subdivision_methods.rb:27
  def create_subdivisions(subdivision_data); end

  # pkg:gem/countries#lib/countries/data/subdivision_methods.rb:16
  def load_data_for_alpha2(alpha2); end

  # pkg:gem/countries#lib/countries/data/subdivision_methods.rb:5
  def subdivision_data(alpha2); end

  # pkg:gem/countries#lib/countries/data/subdivision_methods.rb:21
  def subdivisions(alpha2); end
end

# Extend Country class with support for timezones. Requires the  {tzinfo}[https://github.com/tzinfo/tzinfo] gem
#
#   gem 'tzinfo'
#
# pkg:gem/countries#lib/countries/timezones.rb:8
module ISO3166::TimezoneExtensions
  # TODO: rename method to tz_country or something similar
  #
  # pkg:gem/countries#lib/countries/timezones.rb:10
  def timezones; end
end

# Extend the hash class to allow locale lookup fall back behavior
#
# E.g. if a country has translations for +pt+, and the user looks up +pt-br+ fallback
# to +pt+ to prevent from showing nil values
#
# Also allows "indifferent access" to the hash, so you can use strings or symbols
#
# pkg:gem/countries#lib/countries/translations.rb:10
class ISO3166::Translations < ::Hash
  # pkg:gem/countries#lib/countries/translations.rb:11
  def [](locale); end
end

# pkg:gem/countries#lib/countries/country/class_methods.rb:6
ISO3166::UNSEARCHABLE_METHODS = T.let(T.unsafe(nil), Array)
