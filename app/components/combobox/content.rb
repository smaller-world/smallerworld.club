...
class Components::Combobox::Content < Components::Base
  ...
  sig do
    params(
      attributes: T.untyped,
      content: T.proc.params(list: Components::Combobox::List).void,
    ).void
  end
  def with_list(**attributes, &content)
    @list_block = ->() {
      render Components::Combobox::List.new(**attributes, &content)
    }
  end
