...
module Post::V1Importing
  ...
  sig { params(v1_post: V1::Post, images: T::Array[File]).returns(TrueClass) }
  def update_from_v1_post!(v1_post, images:)
    update!(
      **v1_post.attributes.slice("created_at", "updated_at", "title", "emoji"),
      body: v1_post.body_html,
      images: images.map do |file|
        { io: file, filename: File.basename(file.to_path) }
      end,
  ...
  def reimport_from_v1_post!
    if (v1_post = self.v1_post)
      v1_post.open_ordered_images do |images|
        update_from_v1_post!(v1_post, images:)
      end
    else
      false
    end
